...
router.post('/novo', authenticateToken, async (req, res) => {
    ...
    try {
        const funcionario = await funcionariosService.novoFuncionario(nome, cargo);
...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Erro ao retornar os registros de funcionários!' });
    }
});
...
router.put('/editar/:id', authenticateToken, async (req, res) => {
    ...
    try {
        const funcionario = await funcionariosService.alterarFuncionario(id, nome, cargo);
        res.json(funcionario);
    } catch (error) {
